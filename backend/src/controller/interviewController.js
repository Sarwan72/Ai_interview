...
exports.scheduleInterview = async (req, res) => {
  try {
    ...
    const candidateInfo = await Candidate.findOne({
      $and: [{ email: candidateEmail }, { role: role }],
    });
    if (!candidateInfo) {
      return res.status(404).json({
        success: false,
        message:
          "Candidate information not found. Please create candidate info first.",
      });
    }
    ...
    const sharedLinkId = await getOrCreateSharedLink(
      candidateEmail,
      googleMeetLink,
    );
...
exports.getInterviews = async (req, res) => {
  try {
    const interviews = await Interview.find().populate("sharedLink").exec();
